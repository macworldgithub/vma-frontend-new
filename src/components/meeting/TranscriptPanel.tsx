    ...
    >
      ...
      <div className="flex items-center justify-between p-6 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[10px] font-black text-foreground uppercase tracking-[0.2em]">Live Transcript</h3>
        ...
        <div className="flex items-center gap-1">
          {transcripts.length > 0 && (
            <button
              ...
              className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-all mr-1"
            >
      ...
      {/* Search Filter */}
      <div className="px-6 py-3 border-b border-border bg-muted/20 relative flex items-center">
        <Search className="absolute left-9 h-4 w-4 text-muted-foreground pointer-events-none" />
        <input
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          placeholder="SEARCH CONVERSATION..."
          className="w-full bg-background border border-border rounded-xl pl-10 pr-4 py-2.5 text-[9px] font-bold text-foreground uppercase tracking-widest placeholder:text-muted-foreground focus:outline-none focus:border-primary/30 transition-all"
        />
      </div>
            ...
            return (
              <div key={t.id || i} className="flex flex-col items-start animate-in fade-in duration-200">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className={`text-[10px] font-black uppercase tracking-wider ${isMe ? 'text-primary' : 'text-foreground/80'
                    }`}>
                    {isMe ? 'YOU' : t.userName}
                  </span>
                </div>
                ...
                <div className="pl-7 pr-4">
                  <p className="text-[13px] font-medium text-foreground/90 leading-relaxed tracking-wide break-words first-letter:uppercase">
                    {t.text}
                  </p>
      ...
      <div className="p-4 bg-muted/40 border-t border-border text-[8px] font-black text-muted-foreground text-center uppercase tracking-widest opacity-30 flex items-center justify-center gap-2">
