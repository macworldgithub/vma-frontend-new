...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="relative overflow-hidden sm:rounded-[40px] bg-linear-to-br from-card via-[#020512] to-[#0b1437] border border-primary/10 p-6 sm:p-10 lg:p-14 shadow-2xl shadow-primary/5 animate-pulse-glow">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6 sm:gap-10">
          <div className="space-y-3 sm:space-y-4">
            <div className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-1.5 sm:py-2 bg-accent/5 border border-accent/20 rounded-full w-fit shadow-md shadow-accent/5">
            ...
            <h1 className="text-2xl sm:text-4xl lg:text-6xl font-black text-white uppercase tracking-tighter leading-tight">
              Welcome back, <span className="text-primary">{user?.name?.split(' ')[0]}</span>
            </h1>
            <p className="text-xs sm:text-base lg:text-lg text-slate-400 font-medium max-w-xl">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 w-full lg:w-auto">
            ...
            <Button
              variant="outline"
              className="h-11 sm:h-14 px-4 sm:px-6 rounded-xl sm:rounded-2xl border-white/10 bg-white/5 hover:bg-white/10 gap-2.5 sm:gap-3 text-xs sm:text-sm font-black uppercase tracking-widest w-full sm:w-auto justify-center"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-14 relative z-10 border-t border-white/5 pt-10">
          ...
          <div className="space-y-1">
            ...
            <p className="text-base sm:text-xl font-black text-white uppercase tracking-tight">{meetings.length} Sessions</p>
          </div>
          <div className="space-y-1">
            <p className="text-[8px] sm:text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2">
            ...
            <p className="text-base sm:text-xl font-black text-white uppercase tracking-tight">AU-Regional</p>
          </div>
          <div className="space-y-1">
            <p className="text-[8px] sm:text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2">
            ...
            <p className="text-base sm:text-xl font-black text-white uppercase tracking-tight">Realtime</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 px-2">
        <div className="relative group flex-1 max-w-md">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-500 group-focus-within:text-primary transition-colors" />
          <input
            type="text"
            placeholder="SEARCH SESSIONS BY TITLE OR CODE..."
            className="w-full bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl pl-10 sm:pl-12 pr-4 sm:pr-6 py-3 sm:py-4 text-[9px] sm:text-[10px] font-black text-white uppercase tracking-widest focus:outline-none focus:border-primary/50 transition-all"
          />
        </div>

        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" onClick={fetchMeetings} className="rounded-lg sm:rounded-xl border-white/5 bg-white/5 h-9 sm:h-10 px-3 sm:px-4">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-64 glass rounded-4xl animate-pulse border-white/5 bg-white/5" />
          ))}
        </div>
      ) : meetings.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up">
        ...
        <div className="flex flex-col items-center justify-center py-24 glass rounded-[40px] border-white/5 text-center">
          <div className="relative mb-8">
            ...
            <div className="relative h-24 w-24 bg-slate-900 rounded-4xl border border-white/10 flex items-center justify-center">
              <Video className="h-10 w-10 text-slate-600" />
            </div>
          </div>
          <h3 className="text-xl md:text-2xl font-black text-white uppercase tracking-tighter">No Sessions Found</h3>
