...
function CallbackContent() {
  ...
  if (error) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center p-4 text-center">
        <div className="glass-card bg-card border border-border p-8 rounded-2xl max-w-md w-full shadow-lg">
          ...
          <h1 className="text-2xl font-black text-foreground uppercase mb-2">Connection Failed</h1>
          <p className="text-muted-foreground mb-6">{error}</p>
          <button 
            onClick={() => router.push('/dashboard/calendar')}
            className="w-full bg-primary text-primary-foreground font-bold py-3 rounded-lg hover:bg-primary/90 transition-all uppercase cursor-pointer"
          >
            Back to Calendar
          </button>
        </div>
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-4 text-center">
      <div className="glass-card bg-card border border-border p-10 rounded-2xl max-w-md w-full relative overflow-hidden shadow-lg">
        <div className="absolute top-0 left-0 w-full h-1 bg-primary shimmer" />
        <Loader2 className="w-12 h-12 text-primary animate-spin mx-auto mb-6" />
        <h1 className="text-2xl font-black text-foreground uppercase mb-2">Connecting Account</h1>
